...
const starts = async (dha = new WAConnection()) => {
	...
	console.log(color(figlet.textSync('NT DRAGON', {
		font: 'Standard',
	...
	console.log(color('[ IAGO O BRABO DO MOMENTO ]\n', 'yellow'), color('NT DRAGON DOMINA PORRA!l', 'yellow'))
	console.log(color('NT DRAGON:V', 'red'))
	console.log(color('\n\nNT DRAON REDMORY', 'red'))
	dha.browserDescription = ["herman", "Chrome", "3.0.0"];

	// Menunggu QR
	dha.on('qr', () => {
		console.log(color('[', 'pink'), color('!', 'red'), color(']', 'pink'), color('FACA O SCANEAMENTO DO QR CÓD!'))
	})

	// Menghubungkan
	fs.existsSync(`./${setting.sessionName}.json`) && dha.loadAuthInfo(`./${setting.sessionName}.json`)
	dha.on('connecting', () => {
		console.log(color('NT DRAGON CONCTANDO', 'yellow'), color('PROCESSO DE CONCEÇÃO...'));
	})
const spinner = { 
  "interval": 120,
  "frames": [
	...
	//connect
	dha.on('open', () => {
		console.log(color('conctado', 'blue'), color('BOT RODANDO LIZO'));
	})
